...
セットアップ前のセッションデータを一括送信できます。

**スラッシュコマンド（推奨）:**

プラグインインストール済みなら、Claude Code 内でそのまま実行できます。

```
/kagami-backfill              # 送信
/kagami-backfill --dry-run    # プレビュー（実際には送信しない）
```

**手動実行:**

プラグイン未インストールの環境では、リポジトリから直接実行できます。
...
| コマンド                     | 説明                                     |
| ---------------------------- | ---------------------------------------- |
| `/kagami-backfill`           | 過去セッションデータを kagami API へ送信 |
| `/kagami-backfill --dry-run` | プレビュー（送信せずに結果を確認）       |
